...
\begin{enumerate}
    ...
    \item \textbf{Data Communication and Networking}: It is essential to establish a robust link between CCTV video footage and our backend server. This link will send data to our backend for model inferencing whilst ensuring low latency transmission. Furthermore, our model may be hosted on the cloud via AWS or Google cloud services which would require further data communication within our backend server. Efficient and lossless data transmission will ensure that safety alerts are received with minimum delay and without corruption.
